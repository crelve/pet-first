...
description: Generate Riverpod providers with StateNotifier, AsyncNotifier, and Freezed models. Use when creating state management, adding providers, or working with Riverpod, StateNotifier, Flutter Hooks.

# ❌ BAD: Missing triggers
description: Helps with state management
```
...
```markdown
## Example: Creating a Riverpod Provider

```dart
// Given this requirement: user profile state management
// Generate this Riverpod provider with Freezed model
...
1. **First**: Analyze the existing providers and models
2. **Then**: Ask clarifying questions if needed
3. **Next**: Generate the Dart code following project conventions
4. **Finally**: Run `make update-gen` to generate Freezed/Riverpod code

Always:
- Use `HookConsumerWidget` for widgets
- Use Freezed for immutable state classes
- Follow existing provider patterns in the codebase

Never:
- Use raw `StatefulWidget` (use `HookConsumerWidget`)
- Skip `make update-gen` after adding Freezed models
...
├── validate.py       # Python: Complex validation logic
├── format.py         # Python: Data transformation
└── notify.sh         # Shell: Simple notifications
```
...
description: Generate Flutter widgets with HookConsumerWidget, Riverpod state, and Widget tests. Use when creating new screens, scaffolding UI components, or building widget libraries.
---
...
1. Ask user for widget type (screen/component/dialog)
2. Request widget name and required state
3. Generate:
   - Widget file with HookConsumerWidget
   - Provider file if state management needed
   - Widget test file with comprehensive tests
4. Follow project conventions from existing widgets in lib/screen/ or lib/component/
```
...
1. Analyze project structure and pubspec.yaml
2. Generate sections:
   - Project description
   - Flutter setup steps (FVM, Firebase CLI)
   - Usage examples with `make` commands
...
1. **Description lacks trigger keywords**
   ...
   - Example: Add "flutter_test", "Widget test", "test coverage" to description
...
1. Package skill as pub.dev package or git submodule
...
## Additional Tips for Effective Skills
...
4. **Document Assumptions**: Clarify prerequisites, dependencies, or required knowledge
